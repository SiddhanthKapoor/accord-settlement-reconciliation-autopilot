...
import { getReviewQueue, listRuns, submitReviewAction, verifyChain } from "../api.js";
...
const GROUPS = [
  ...
  { key: "HIGH", label: "High priority", match: (i) => i.severity === "HIGH" },
  { key: "MONEY", label: "Money dispute", match: (i) => MONEY_DISPUTES.has(i.exception_type) },
  {
    key: "AMBIGUOUS",
    label: "Ambiguous",
    match: (i) =>
  ...
  {
    // The queue's real reason to exist: evidence is on the table, but the
    // engine would not sign it off.
    key: "UNMATCHED",
    label: "Candidates, unmatched",
    match: (i) =>
      !i.matched_payment_id &&
      ((i.candidates || []).length > 0 ||
        (i.considered_candidates || []).some((c) => c.admissible)),
  },
  {
    key: "NOEVIDENCE",
    label: "Missing evidence",
    match: (i) => i.classification === "NO_ADMISSIBLE_CANDIDATE",
  },
];
...
export default function ReviewQueue() {
  ...
  const load = useCallback((id) => {
    getReviewQueue({ batchId: id || undefined, limit: 50 })
      .then((data) => {
        setQueue(data);
        setError(null);
      })
  ...
  const counts = useMemo(() => {
    const out = {};
    for (const g of GROUPS) out[g.key] = items.filter(g.match).length;
    return out;
  }, [items]);
  const shown = useMemo(() => {
    const g = GROUPS.find((x) => x.key === group) || GROUPS[0];
    return items.filter(g.match);
  }, [items, group]);
          ...
          {Object.entries(summary.by_exception_type || {})
            ...
            .map(([type, stats]) => (
              <div className="pn-metric" key={type}>
                ...
                <span className="pn-metric-note">{money(stats.amount_minor || 0, { decimals: 0 })}</span>
              </div>
            ))}
        </div>
      )}
        ...
        <div className="pn-chips" role="group" aria-label="Filter the queue">
          {GROUPS.map((g) => (
            <button
              ...
              disabled={g.key !== "ALL" && counts[g.key] === 0}
      ...
      <div className="pn-section">
        <div className="pn-section-head">
          <h2 className="pn-section-title">
            {queue === null
              ? "Loading"
              : `${shown.length} of ${items.length} awaiting review`}
          </h2>
